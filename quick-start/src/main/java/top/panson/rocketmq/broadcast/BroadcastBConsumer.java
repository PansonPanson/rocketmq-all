...
public class BroadcastBConsumer implements RocketMQListener<BroadcastMessage> {
    ...
    public void onMessage(BroadcastMessage message) {
        logger.info("[BroadcastBConsumer-onMessage][线程编号:{} 消息内容：{}]", Thread.currentThread().getId(), message);
    }
}
